<!DOCTYPE html>
<html lang="pt-br">
<head>
    <meta charset="UTF-8">
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <link rel='stylesheet' type='text/css' media='screen' href='../../css/css.css'>
    <link rel="shortcut icon" href="../../imgs/livro3.png"/>
    <title> Harry Potter </title>
</head>

<body>
<div class='cabeçalho'>
    <h1 class="titulo"> IF LEITURA </h1>
    <ul>
    <li> <a href="../home.php"> LIVROS </a> </li>
    <li > <a href="../cadastroF.php"> CADASTRO </a> </li>
    <li> <a href="../loginF.php"> LOGIN </a> </li>
    <li> <a href="../index.php">SAIR</a> </li>
    </ul>
</div>

<div class="livro-pag">

    <div class="conteudo-livro">
        <h2> Harry Potter e a Pedra Filosofal </h2> <h3> J.K. Rowling </h3> <br>
        <img src="../../imgs/harry.jpg" alt="Harry Potter e a Pedra Filosofal" class="capa-livro"/> <br>
        <h3> Sinopse do livro </h3>
        <p class="sinopse"> traiu ou não traiu? </p>
    </div>

    <div class="resenha">
        <h2> Adicione sua resenha/avaliação sobre essa obra! </h2> <br>
        <input type="text" class="resenha-input" placeholder="E">
    </div>
</div>


</body>
</html>